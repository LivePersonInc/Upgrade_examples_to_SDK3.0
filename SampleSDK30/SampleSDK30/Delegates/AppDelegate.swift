...
class AppDelegate: UIResponder, UIApplicationDelegate, LPMessagingSDKNotificationDelegate {
  ...
  var window: UIWindow?
  // TODO: Change flag in ViewController.swift to match this one
  ...
  func applicationDidBecomeActive(_ application: UIApplication) {
    ...
    // Check if Application was open from a Push Notification
    if application.applicationState == .background || application.applicationState == .inactive {
      // Change Push Flag
      self.comesFromPush = true
    }
  }
  ...
  private func showMessaging(){
    ...
    if LivePersonSDK.shared.isWindowMode {
      ...
      // Change Push Flag
      mainViewController?.comesFromPush = true
