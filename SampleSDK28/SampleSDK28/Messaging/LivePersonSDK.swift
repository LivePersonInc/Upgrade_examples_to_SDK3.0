...
class LivePersonSDK: NSObject {
  ...
  public func showConversation(){
    ...
    if self.conversationQuery != nil {
      // INFO : LPMessagingSDK Version 2.7 introduced LPConversationViewParams() to handle ConversationParamProtocol, ViewController reference, and Viewing Permissions (False - View only)
  ...
  public func reconnect(){
    // Check if ConversationQuery has been set
    if self.conversationQuery != nil {
      // INFO : LPMessagingSDK Version 2.7 introduced LPAuthenticationParams() to handle Authentication Parameters and a new reconnect Method
